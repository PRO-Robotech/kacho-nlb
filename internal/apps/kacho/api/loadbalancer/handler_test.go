...
func TestHandler_DispatchesAll(t *testing.T) {
	...
	h := NewHandler(repo, opsRepo, &fakeProjectClient{}, &fakeRegionClient{}, &fakeZoneClient{}, &fakeSubnetClient{}, &fakeAddressReader{}, &fakeAddressClient{}, nil, slog.Default())
...
func TestHandler_NewHandler_NilLogger_OK(t *testing.T) {
	t.Parallel()
	h := NewHandler(newFakeRepo(), newFakeOpsRepo(), nil, nil, nil, nil, nil, nil, nil, nil)
	require.NotNil(t, h)
}

func TestHandler_Get_PropagatesErr(t *testing.T) {
	t.Parallel()
	h := NewHandler(newFakeRepo(), newFakeOpsRepo(), nil, nil, nil, nil, nil, nil, nil, slog.Default())
