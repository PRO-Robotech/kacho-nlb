...
// blockingAddressService — fake AddressServiceServer that never returns from
// Get until explicitly released (simulates a hung/stalled kacho-vpc peer).
type blockingAddressService struct {
	vpcpb.UnimplementedAddressServiceServer
	release chan struct{}
}

func (f *blockingAddressService) Get(_ context.Context, _ *vpcpb.GetAddressRequest) (*vpcpb.Address, error) {
	<-f.release
	return &vpcpb.Address{Id: "e9b-ip-1"}, nil
}

// TestAddressClient_Get_HangingPeer_BoundsToConfiguredTimeout — regression for
...
func TestAddressClient_Get_HangingPeer_BoundsToConfiguredTimeout(t *testing.T) {
	fake := &blockingAddressService{release: make(chan struct{})}
	conn := startFakeVPC(t, nil, nil, fake, nil, nil)

	const configuredTimeout = 100 * time.Millisecond
	c := NewAddressClientWithTimeout(conn, configuredTimeout)

	start := time.Now()
	_, err := c.Get(context.Background(), "e9b-ip-1")
