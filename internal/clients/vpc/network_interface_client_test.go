...
// blockingNICService — fake NetworkInterfaceServiceServer that never returns
// from Get until explicitly released (simulates a hung/stalled kacho-vpc peer).
type blockingNICService struct {
	vpcpb.UnimplementedNetworkInterfaceServiceServer
	release chan struct{}
}

func (f *blockingNICService) Get(
	_ context.Context, _ *vpcpb.GetNetworkInterfaceRequest,
) (*vpcpb.NetworkInterface, error) {
	<-f.release
	return &vpcpb.NetworkInterface{Id: "e9b-nic-1"}, nil
}

// TestNetworkInterfaceClient_Get_HangingPeer_BoundsToConfiguredTimeout —
// regression for the missing per-call deadline (round-6 audit finding 2,
// sibling client): a stalled kacho-vpc peer must not park the calling
// goroutine forever. Get is called with a deadline-less caller ctx
...
func TestNetworkInterfaceClient_Get_HangingPeer_BoundsToConfiguredTimeout(t *testing.T) {
	fake := &blockingNICService{release: make(chan struct{})}
	conn := startFakeVPC(t, nil, fake, nil, nil, nil)

	const configuredTimeout = 100 * time.Millisecond
	c := NewNetworkInterfaceClientWithTimeout(conn, configuredTimeout)

	start := time.Now()
	_, err := c.Get(context.Background(), "e9b-nic-1")
